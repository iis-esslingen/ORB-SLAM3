...
class SO3 : public SO3Base<SO3<Scalar_, Options>> {
 public:
  ...
  SOPHUS_FUNC static SO3<Scalar> expAndTheta(Tangent const& omega,
    ...
    if (omega.hasNaN() || !omega.allFinite()) {
      // SOPHUS_ENSURE(false, "Invalid omega input: contains NaN or Inf.");
      return SO3(QuaternionMember(Scalar(1), Scalar(0), Scalar(0),
                                  Scalar(0)));  // Identity SO3
    }
    ...
    if (theta_sq <
      ...
      *theta = sqrt(theta_sq);
      ...
      // Add safeguard for small theta values to avoid division by zero
      if (*theta < Constants<Scalar>::epsilon()) {
        *theta < Constants<Scalar>::epsilon();
      }
