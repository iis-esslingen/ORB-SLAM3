...
int main(int argc, char** argv)
{
  ...
  {
    cerr << endl
         << "Usage: ./mono_inertial_realsense_t265 path_to_vocabulary path_to_settings (trajectory_file_name)" << endl;
    return 1;
  }
  ...
  while (!SLAM.isShutDown())
  {
...
#endif
        t_resize = std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(t_End_Resize - t_Start_Resize)
                       .count();
    ...
    for (int i = 0; i < vGyro.size(); ++i)
    {
      ...
      if (isnan(vAccel[i].x) || isnan(vAccel[i].y) || isnan(vAccel[i].z) || isnan(vGyro[i].x) || isnan(vGyro[i].y) ||
          isnan(vGyro[i].z) || isnan(vGyro_times[i]))
      {
    ...
    t_track =
        t_resize +
        std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(t_End_Track - t_Start_Track).count();
    SLAM.InsertTrackTime(t_track);
#endif

    double timeProcess =
        std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(t_Start_Track - time_Start_Process)
            .count();
    double timeSLAM =
        std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(t_End_Track - t_Start_Track).count();
