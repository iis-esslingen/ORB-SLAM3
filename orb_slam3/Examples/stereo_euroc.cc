...
int main(int argc, char** argv)
{
  ...
  {
    cerr << endl
         << "Usage: ./stereo_euroc path_to_vocabulary path_to_settings path_to_sequence_folder_1 path_to_times_file_1 "
  ...
  for (seq = 0; seq < num_seq; seq++)
  {
    ...
    for (int ni = 0; ni < nImages[seq]; ni++, proccIm++)
    {
      ...
      t_track =
          t_resize + t_rect + std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(t2 - t1).count();
