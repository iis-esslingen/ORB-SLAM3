...
void Tracking::PrintTimeStats(const string& filename)
{
  // Save data in files
  TrackStats2File();
  LocalMapStats2File();
...
void Tracking::ExtrapolateIMUData()
{
  if (mvImuFromLastFrame.size() > 0)
  {
    IMU::Point lastImuMeasurement = mvImuFromLastFrame.back();

    // Extrapolate assuming constant velocity and acceleration
    Eigen::Vector3f extrapolatedAcc = lastImuMeasurement.a;     // Assume acceleration remains constant
    Eigen::Vector3f extrapolatedAngVel = lastImuMeasurement.w;  // Assume angular velocity remains constant
    float tstep = mCurrentFrame.mTimeStamp - mLastFrame.mTimeStamp;

    // Integrate this extrapolated measurement
    mpImuPreintegratedFromLastKF->IntegrateNewMeasurement(extrapolatedAcc, extrapolatedAngVel, tstep);

    // Create a new Preintegrated object for the current frame
    IMU::Preintegrated* pImuPreintegratedFromLastFrame =
        new IMU::Preintegrated(mLastFrame.mImuBias, mCurrentFrame.mImuCalib);
    pImuPreintegratedFromLastFrame->IntegrateNewMeasurement(extrapolatedAcc, extrapolatedAngVel, tstep);

    // Assign the preintegrated IMU data to the current frame
