...
void save_map(const string& file_name)
{
  bool success = pSLAM->SaveMap(file_name);

  if (success)
    ROS_INFO("Map was saved as %s.osa", file_name);
  else
    ROS_ERROR("Map could not be saved.");
}
...
void save_traj(const string& file_name)
{
  try
  {
    pSLAM->SaveTrajectoryTUM(file_name);
    ROS_INFO("Estimated trajectory saved successfully to %s.", &file_name);
  }
  catch (const std::exception& e)
  {
    std::cerr << e.what() << std::endl;
...
void setup_publishers(ros::NodeHandle& node_handler, image_transport::ImageTransport& image_transport,
